...
function onEvent(name, value1, value2)
	if name == "Must Press Swap" then
		for i = 0, 3 do
			...
			noteTweenX('note'..i..'TwnX', i, iPos, 0.35, 'expoInOut');
			noteTweenX('note'..j..'TwnX', j, jPos, 0.35, 'expoInOut');
		end
		...
		if not lowQuality then
			for i = 0, 7 do
				...
				doTweenAlpha(tag..'alphaTweenThing', tag, 0.5, 0.25, 'expoInOut');
			end
...
function onTimerCompleted(tag, loops, loopsLeft)
	if tag == 'removeTrailThing' then
		for i = 0, 7 do
			tag = 'psychicNoteTrail'..i;
			doTweenAlpha(tag..'alphaTweenThing', tag, 0.0, 0.2, 'expoInOut');
		end
	end
end